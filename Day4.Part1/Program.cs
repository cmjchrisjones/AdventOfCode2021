...
////var puzzleInput = new int[] { 22, 8, 21, 6, 1, 7, 4, 9, 5, 11, 17, 23, 2, 0, 14, 21, 24, 10, 16, 13, 6, 15, 25, 12, 22, 18, 20, 8, 19, 3, 26, 1 };
...
for (var i = 0; i < cardNumbersAsInt.ToArray().Length; i += 25)
{
    var card = new BingoCard(cardNumbersAsInt.ToList().Skip(i).Take(25).ToList(), cardInteger);
    card.Print();
    bingoCards.Add(card);
    cardInteger++;
}

PlayBingo(bingoCards, bingoNumbers.ToArray());
